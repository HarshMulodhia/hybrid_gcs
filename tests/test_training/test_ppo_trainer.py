...
class TestPPOConfig:
    ...
    def test_creation_custom(self):
        """Test custom config."""
        config = PPOConfig(learning_rate=1e-3, gamma=0.95, clip_ratio=0.1)
...
class TestPPOTrainer:
    ...
    def setup(self):
        ...
        policy_config = PolicyNetworkConfig(
            state_dim=6, action_dim=2, hidden_dim=64, num_hidden_layers=2, device=device
        )
    ...
    def test_checkpoint_save_load(self, setup, tmp_path):
        ...
        policy_config = PolicyNetworkConfig(state_dim=6, action_dim=2, hidden_dim=64, device="cpu")
...
class TestPPOTrainerIntegration:
    ...
    def test_training_loop(self):
        ...
        # Setup
        policy_config = PolicyNetworkConfig(state_dim=4, action_dim=2, hidden_dim=32, device=device)
        policy = PolicyNetwork(policy_config)

        ppo_config = PPOConfig(learning_rate=1e-3, epochs=2, batch_size=16, device=device)
        trainer = PPOTrainer(policy, ppo_config)
    ...
    def test_policy_improvement(self):
        ...
        policy_config = PolicyNetworkConfig(state_dim=2, action_dim=1, hidden_dim=32, device=device)
        policy = PolicyNetwork(policy_config)
