    ...
    def update(self, performance: float):
        ...
        if performance >= self.success_threshold:
            ...
            self.current_difficulty = min(
                self.current_difficulty + self.increase_rate, self.max_difficulty
            )
        elif performance < self.failure_threshold:
            # Decrease difficulty
            self.current_difficulty = max(self.current_difficulty - self.decrease_rate, 0.0)
...
class CurriculumManager:
    ...
    def __init__(self, schedule: CurriculumSchedule = None, config: CurriculumConfig = None):
        """
    ...
    def _create_schedule(self, config: CurriculumConfig) -> CurriculumSchedule:
        ...
        if config.schedule_type == CurriculumType.LINEAR:
            return LinearCurriculum(
                config.initial_difficulty, config.final_difficulty, config.total_steps
            )
        elif config.schedule_type == CurriculumType.EXPONENTIAL:
            return ExponentialCurriculum(
                config.initial_difficulty, config.final_difficulty, config.total_steps, config.rate
            )
