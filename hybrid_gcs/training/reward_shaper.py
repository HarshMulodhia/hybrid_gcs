...
def create_reward_composer(config: RewardConfig) -> RewardComposer:
    ...
    if config.use_distance_reward:
        composer.add_strategy(
            DistanceReward(config.distance_weight, config.distance_scale), "distance"
        )
    ...
    if config.use_action_penalty:
        composer.add_strategy(
            ActionPenalty(config.action_weight, config.action_scale), "action_penalty"
        )

    if config.use_collision_penalty:
        composer.add_strategy(
            CollisionPenalty(config.collision_weight, config.collision_penalty), "collision"
        )

    if config.use_smoothness_reward:
        composer.add_strategy(
            SmoothnessReward(config.smoothness_weight, config.smoothness_scale), "smoothness"
        )

    if config.use_efficiency_reward:
        composer.add_strategy(
            EfficiencyReward(config.efficiency_weight, config.efficiency_scale), "efficiency"
        )
