...
class ExperienceBuffer:
    ...
    def __init__(
        self, capacity: int = 10000, state_dim: int = 6, action_dim: int = 2, device: str = "cpu"
    ):
        """
    ...
    def add_trajectory(self, trajectory: Dict[str, np.ndarray]):
        ...
        for t in range(T):
            self.add(
                ...
                next_state=trajectory["states"][t + 1] if t + 1 < T else np.zeros(self.state_dim),
    ...
    def sample_sequence(self, length: int, batch_size: int = 1) -> Dict[str, torch.Tensor]:
        """
        ...
        return {
            "states": torch.FloatTensor(self.states[all_indices])
            .to(self.device)
            .reshape(batch_size, length, self.state_dim),
            "actions": torch.FloatTensor(self.actions[all_indices])
            .to(self.device)
            .reshape(batch_size, length, self.action_dim),
            "rewards": torch.FloatTensor(self.rewards[all_indices])
            .to(self.device)
            .reshape(batch_size, length),
            "dones": torch.BoolTensor(self.dones[all_indices])
            .to(self.device)
            .reshape(batch_size, length),
        }
...
class TrajectoryBuffer:
    ...
    def sample_trajectory(self) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "log_probs": torch.FloatTensor(traj.get("log_probs", np.zeros(len(traj["states"])))).to(
                self.device
            ),
            "values": torch.FloatTensor(traj.get("values", np.zeros(len(traj["states"])))).to(
                self.device
            ),
        }
