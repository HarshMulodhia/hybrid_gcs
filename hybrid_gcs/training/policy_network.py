...
class PolicyNetwork(nn.Module):
    ...
    def __init__(self, config: PolicyNetworkConfig):
        ...
        self.actor_log_std = nn.Parameter(torch.ones(config.action_dim) * config.log_std_init)
    ...
    def forward(
        ...
        log_std = self.actor_log_std.clamp(min=self.config.min_log_std, max=self.config.max_log_std)
    ...
    def set_log_std(self, log_std: torch.Tensor):
        ...
        self.actor_log_std.data = log_std.clamp(
            min=self.config.min_log_std, max=self.config.max_log_std
        )
...
class PolicyNetworkWithLSTM(nn.Module):
    ...
    def forward(
        self, state: torch.Tensor, hidden_state: Optional[Tuple] = None
