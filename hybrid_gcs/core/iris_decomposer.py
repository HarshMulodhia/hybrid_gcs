...
class IRISDecomposer:
    ...
    def decompose(
        self, seed_points: List[np.ndarray], obstacles: Optional[List] = None, max_regions: int = 50
    ) -> List[Ellipsoid]:
        """
    ...
    def _grow_ellipsoid(self, center: np.ndarray, obstacles: List) -> Optional[Ellipsoid]:
        """
    ...
    def _min_distance_to_obstacles(
        self, center: np.ndarray, Q: np.ndarray, obstacles: List
    ) -> float:
        """
        ...
        for obstacle in obstacles:
            ...
            if cp is not None:
                ...
                if hasattr(obstacle, "cvxpy_constraints") and callable(
                    getattr(obstacle, "cvxpy_constraints")
                ):
                    try:
    ...
    def _in_collision(self, point: np.ndarray, obstacles: List) -> bool:
        """
