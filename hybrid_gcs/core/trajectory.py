...
class Trajectory:
    ...
    def __init__(self, waypoints: np.ndarray, timestamps: Optional[np.ndarray] = None):
        """
        ...
        if timestamps is None:
            ...
            if len(self.timestamps) != n_waypoints:
                raise ValueError(
                    f"timestamps length {len(self.timestamps)} != " f"waypoints {n_waypoints}"
                )

        # Build cubic spline for smooth interpolation
        self.spline = CubicSpline(self.timestamps, self.waypoints, extrapolate="clip")
    ...
    def resample(self, n_waypoints: int) -> "Trajectory":
        ...
        new_times = np.linspace(self.timestamps[0], self.timestamps[-1], n_waypoints)
    ...
    def smooth(self, smoothing_factor: float = 0.1) -> "Trajectory":
        ...
        for i in range(self.dim):
            smoothed_waypoints[:, i] = np.convolve(
                self.waypoints[:, i], np.ones(window_size) / window_size, mode="same"
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Trajectory(waypoints={len(self)}, "
            f"dim={self.dim}, "
...
class BezierTrajectory:
    ...
    def __init__(self, control_points: np.ndarray, degree: Optional[int] = None):
        ...
        if self.control_points.ndim != 2:
            raise ValueError(f"control_points must be 2D, got {self.control_points.ndim}D")
    ...
    def derivative(self, t: float, order: int = 1) -> np.ndarray:
        ...
        if order > 1:
            new_bezier = BezierTrajectory(
                np.diff(self.control_points, axis=0) * self.degree, degree=self.degree - 1
            )
    ...
    def __repr__(self) -> str:
        """String representation."""
        return (
            f"BezierTrajectory(degree={self.degree}, "
            f"dim={self.dim}, "
