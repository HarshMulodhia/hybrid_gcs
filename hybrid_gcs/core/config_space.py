...
class ConfigSpace:
    ...
    def is_valid(self, q: np.ndarray) -> bool:
        ...
        return (
            q.shape == (self.dim,)
            and np.all(q >= self.bounds_lower)
            and np.all(q <= self.bounds_upper)
        )
    ...
    def random_samples(self, n: int) -> np.ndarray:
        ...
        """
        return np.random.uniform(self.bounds_lower, self.bounds_upper, size=(n, self.dim))
    ...
    def interpolate(self, q1: np.ndarray, q2: np.ndarray, t: float) -> np.ndarray:
        """
